...
export function BillProvider({ children }) {
  ...
  const createItemKey = () => {
    ...
    let key = uuidv4(); // ⇨ '9b1deb4d-3b7d-4bad-9bdd-2b0d7b3dcb6d'
  ...
  const handleLoadInvoiceDetail = async (id) => {
    ...
    console.log(data);
    data.items = addUniqueKeys(data.items);
    data.orders = addUniqueKeys(data.orders);
    data.orders = data.orders.map(order =>
      {
        debugger;
         order.items = addUniqueKeys(order.items, id = order.id)
        return order;
      });
    if (ok) {
